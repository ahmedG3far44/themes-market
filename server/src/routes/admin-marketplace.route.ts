...
import { discountInputSchema, discountPatchSchema, idSchema, parseOrThrow, themeInputSchema, themePatchSchema, uploadCompleteSchema, uploadInitSchema, uploadPartSchema } from "../schemas/marketplace.ts";
...
router.post("/discounts", async (req, res, next) => { try { const input = parseOrThrow(discountInputSchema, req.body); const item = await DiscountModel.create({ ...input, createdBy: req.currentUser!._id }); await audit(req, "discount.create", "discount", String(item._id), undefined, item.toObject()); res.status(201).json({ success: true, data: item, message: "Discount created" }); } catch (error) { next(error); } });
router.put("/discounts/:id", async (req, res, next) => { try { const { id } = parseOrThrow(idSchema, req.params); const before = await DiscountModel.findById(id).lean(); if (!before) throw new AppError(404, "DISCOUNT_NOT_FOUND", "Discount not found"); const item = await DiscountModel.findByIdAndUpdate(id, parseOrThrow(discountPatchSchema, req.body), { returnDocument: "after", runValidators: true }); await audit(req, "discount.update", "discount", id, before, item?.toObject()); res.json({ success: true, data: item, message: "Discount updated" }); } catch (error) { next(error); } });
router.delete("/discounts/:id", async (req, res, next) => { try { const { id } = parseOrThrow(idSchema, req.params); const item = await DiscountModel.findByIdAndDelete(id); if (!item) throw new AppError(404, "DISCOUNT_NOT_FOUND", "Discount not found"); await audit(req, "discount.delete", "discount", id, item.toObject()); res.json({ success: true, data: null, message: "Discount deleted" }); } catch (error) { next(error); } });
