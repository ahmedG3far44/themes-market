...
router.post("/discount", async (req, res, next) => { try { const { code } = parseOrThrow(cartDiscountSchema, req.body); res.json({ success: true, data: await applyCartDiscount(req.currentUser!._id, code, req.ip), message: "Discount applied" }); } catch (error) { next(error); } });
router.delete("/discount", async (req, res, next) => { try { res.json({ success: true, data: await clearCartDiscount(req.currentUser!._id, req.ip) }); } catch (error) { next(error); } });
