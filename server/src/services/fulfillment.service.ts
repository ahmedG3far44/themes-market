...
export async function fulfillPaidOrder(order: OrderDocument & { _id: unknown }, externalId?: string, paymentIntentId?: string): Promise<void> {
  ...
  for (const item of order.items) {
    const result = await EntitlementModel.updateOne(
      ...
      { $setOnInsert: { orderId: order._id, orderItemId: item._id, purchasedVersion: item.version, sourceAssetId: item.sourceAssetId, downloadLimit: 5, downloadsUsed: 0, status: "active", purchasedAt: paidAt } },
      { upsert: true },
    );
...
export async function failOrderPayment(order: OrderDocument & { _id: unknown }, externalId?: string): Promise<void> {
  await Promise.all([
    ...
    TransactionModel.updateOne({ orderId: order._id, provider: order.paymentProvider }, { $set: { status: "declined", ...(externalId ? { externalId } : {}) } }),
  ]);
}
