...
export function CartProvider({ children }: { children: ReactNode }) {
  const { user } = useAppAuth(); const { notify } = useToast(); const [cart, setCart] = useState<CartType | null>(null); const [isLoading, setLoading] = useState(false);
  const canPurchase = Boolean(user && user.role !== "admin");
  const refresh = useCallback(async () => { if (!canPurchase) { setCart(null); return; } setLoading(true); try { setCart(await api.get<CartType>("/cart")); } finally { setLoading(false); } }, [canPurchase]);
  useEffect(() => { if (canPurchase) void refresh().catch(() => undefined); else setCart(null); }, [canPurchase, refresh]);
